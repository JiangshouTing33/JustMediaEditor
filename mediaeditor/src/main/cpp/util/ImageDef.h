...
#ifndef JUSTMEDIAEDITOR_IMAGEDEF_H
#define JUSTMEDIAEDITOR_IMAGEDEF_H
...
typedef struct _tag_FrameImage
{
    int width;
    int height;
    int format;
...
public:
    static void AllocFrameImage(FrameImage *pImage) {
        ...
        switch (pImage->format) {
            ...
            case IMAGE_FORMAT_I420:
                ...
                break;
            default:
                LOGCATE("FrameImageUtil::AllocFrameImage do not support the format. Format = %d", pImage->format);
                break;
        }
    }

    static void FreeFrameImage(FrameImage *pImage) {
    ...
    static void CopyFrameImage(FrameImage *pDstImg, FrameImage *pSrcImg) {
        ...
        || pSrcImg->height != pDstImg->height) {
            LOGCATE("FrameImageUtil::CopyFrameImage invalid params.");
            return;
        }

        if(pDstImg->ppPlane[0] == nullptr) AllocFrameImage(pDstImg);
        ...
        switch (pSrcImg->format) {
            ...
            case IMAGE_FORMAT_RGBA:
                ...
                break;
            default:
                LOGCATE("FrameImageUtil::CopyFrameImage do not support the format. Format = %d", pSrcImg->format);
                break;
        }
    }

    static void DumpFrameImage(FrameImage *pSrcImg, const char *pPath, const char *pFileName)
    {
        ...
        LOGCATD("DumpFrameImage fp=%p, file=%s", fp, imgPath);
        ...
        if(fp) {
            switch (pSrcImg->format) {
                ...
                case IMAGE_FORMAT_RGBA:
                    ...
                    break;
                default:
                    LOGCATE("DumpFrameImage default");
                    break;
            }
            fclose(fp);
            fp = NULL;
...
#endif //JUSTMEDIAEDITOR_IMAGEDEF_H
